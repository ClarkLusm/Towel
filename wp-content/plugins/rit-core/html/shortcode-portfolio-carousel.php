...
<div id="<?php echo esc_attr($ri_wrapID) ?>" class="rit-portfolio-carousel rit-projects ">
    ...
    if (!empty($ri_terms) && !is_wp_error($ri_terms)):?>
        <div class="wrap-proj-filter">
            ...
            <ul id="project-filter" class="special-font text-left">
                <?php if (count($ri_terms) > 1) : ?>
                    <li class="active" data-id="all"><span class="first hvr-underline-from-center"><?php echo esc_html__('All', 'ri-dilo') ?></span></li>
                    <?php foreach ($ri_terms as $ri_term) : ?>
                        <li data-id="<?php echo esc_attr($ri_term->slug) ?>">
                            <span class="hvr-underline-from-center"><?php echo esc_html($ri_term->name); ?></span></li>
                    <?php endforeach; ?>
                <?php endif; ?>
            </ul>
        </div>
    ...
    <div class="row">
        <div class="wrap-proj-block-item">
        <?php
        while ($ri_the_query->have_posts()): $ri_the_query->the_post();
            ...
            ?>
            <article id="portfolio-<?php esc_attr(the_ID()); ?>" <?php echo post_class($ri_class . $ri_catslug); ?>
                ...
                <div class="wrap-project-item">
                    ...
                    <div class="proj-info rit-info-grid">
                        <h3 class="title title-post">
                            <a href="<?php the_permalink(); ?>" class="hvr-underline-from-center-img" title="<?php the_title(); ?>"><?php the_title(); ?></a>
                        </h3>
...
<script type="text/javascript">
    (function ($) {
        ...
        $(document).ready(function () {
            var wrapID='<?php echo esc_js('#'.$ri_wrapID);?>';
           var slider=jQuery("#<?php echo (esc_js($ri_wrapID));?> .wrap-proj-block-item");
                slider.owlCarousel({
                    // Most important owl features
                    items: <?php echo esc_js($atts['columns']) ?>,
                    itemsCustom: false,
                    itemsDesktop: [1199, <?php echo esc_js($atts['columns']) ?>],
                    ...
                    autoPlay:3000,
                    afterMove:function(){
                        var realwidth=$(wrapID).find('.owl-item').not('.outfilter').length*$(wrapID).find('.owl-item').width();
                        var maxwidth=parseInt(realwidth-$(wrapID).find('.owl-wrapper-outer').width());
                        var currTrans =$(wrapID).find(".owl-wrapper").css('transform').split(/[()]/)[1];
                        var posx =  parseInt(currTrans.split(',')[4]);
                        if((maxwidth+posx)<0 || (maxwidth+posx)==0){
                            slider.data('owlCarousel').goTo(0);
                        }
                    }
                });
            $(wrapID).find('.project-item').each(function(){
                $(this).height($(this).height());
                $(this).width($(this).width())
            });
            $(wrapID+' #project-filter li').on('click', function () {
                $(wrapID+' #project-filter li').removeClass('active');
                $(this).addClass('active');
                $(wrapID+' #mobile-proj-filter span').text($(this).text());
                var filtervar = $(this).attr('data-id');
                $(wrapID).find('.outfilter').removeClass('outfilter');
                $(wrapID).find('.project-item:not(.'+filtervar+')').parent().addClass('outfilter');
                slider.data('owlCarousel').goTo(0);
            });
            $(wrapID+' #project-filter').on('click',function(){
                $(this).removeClass('active');
            });
            $(wrapID+' #mobile-proj-filter').on('click',function(){
                $(wrapID+' #project-filter').toggleClass('active');
            });
            $(wrapID+' .owl-buttons div ').on('click',function(){
                var realwidth=$(wrapID).find('.owl-item').not('.outfilter').length*$(wrapID).find('.owl-item').width();
                var maxwidth=parseInt(realwidth-$(wrapID).find('.owl-wrapper-outer').width());
                var currTrans =$(wrapID).find(".owl-wrapper").css('transform').split(/[()]/)[1];
                var posx =  parseInt(currTrans.split(',')[4]);
                if($(this).hasClass('owl-prev')){
                    if($(wrapID).find('.owl-wrapper').width()>realwidth*2&&posx==0){
                        if($(window).width()>768){
                            slider.data('owlCarousel').goTo($(wrapID).find('.owl-item').not('.outfilter').length - <?php echo esc_js($atts['columns']) ?>);
                        }else if(480 < $(window).width() <768){
                            slider.data('owlCarousel').goTo($(wrapID).find('.owl-item').not('.outfilter').length - 2);
                        }
                        else{
                            slider.data('owlCarousel').goTo($(wrapID).find('.owl-item').not('.outfilter').length - 1);
                        }
                    }
                }
            })

...
if (!class_exists('RITSocialWidget')) {
    class RITSocialWidget extends WP_Widget
    {
        ...
        {
            $widget_ops = array('classname' => 'RITSocialWidget', 'description' => esc_html__('Displays your social profile.', 'ri-dilo'));

            parent::__construct(false, esc_html__('RIT Social', 'ri-dilo'), $widget_ops);
        }
        ...
        function widget($args, $instance)
        {
            ...
            echo '<div class="rit-widget-social-icon clearfix">';
            foreach ($this->socials as $key => $social) {
                if (!empty($instance[$social['name']])) {
                    echo '<a href="' . str_replace('*', esc_attr($instance[$social['name']]), $social['link']) . '" target="_blank" title="' . esc_attr($key) . '" class="' . esc_attr($key.' social-icon') . '"><i class="fa ' . esc_attr( $social['icon']) . '"></i></a>';
                }
            }
            echo '</div>';

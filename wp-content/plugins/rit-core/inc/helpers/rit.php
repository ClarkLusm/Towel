...
if( ! function_exists('ri_wpdocs_custom_excerpt_length')){
function ri_wpdocs_custom_excerpt_length( $ri_length ) {
    return 30;
} }
add_filter( 'excerpt_length', 'ri_wpdocs_custom_excerpt_length', 999 );

/*==========search_form_modify============*/
if( ! function_exists('ri_dilo_search_form_modify')){
function ri_dilo_search_form_modify( $ri_html ) {
    return str_replace( 'class="search-submit"', 'class="search-submit screen-reader-text"', $ri_html );
} }
add_filter( 'get_search_form', 'ri_dilo_search_form_modify' );

// Author Link Social
if (!function_exists('rit_dilo_social_author')) {
    function rit_dilo_social_author($contactmethods)
    {
        $contactmethods['twitter'] = esc_html__('Twitter Username', 'ri-dilo');
        $contactmethods['facebook'] = esc_html__('Facebook Username', 'ri-dilo');
        $contactmethods['google'] = esc_html__('Google Plus Username', 'ri-dilo');
        $contactmethods['tumblr'] = esc_html__('Tumblr Username', 'ri-dilo');
        $contactmethods['instagram'] = esc_html__('Instagram Username', 'ri-dilo');
        $contactmethods['pinterest'] = esc_html__('Pinterest Username', 'ri-dilo');

        return $contactmethods;
    }
}

add_filter('user_contactmethods', 'rit_dilo_social_author', 10, 1);
